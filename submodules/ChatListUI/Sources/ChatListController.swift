...
public class ChatListControllerImpl: TelegramBaseController, ChatListController {
    ...
    public init(context: AccountContext, location: ChatListControllerLocation, controlsHistoryPreload: Bool, hideNetworkActivityStatus: Bool = false, previewing: Bool = false, enableDebugActions: Bool, animationInController: ContextAnimationInControllerProtocol, animationOutController: ContextAnimationOutControllerProtocol) {
        ...
        self.chatListDisplayNode.mainContainerNode.groupSelected = { [weak self] groupId in
            ...
            |> deliverOnMainQueue).start(next: { [weak self] didDisplayTip, settings in
                ...
                if let navigationController = self.navigationController as? NavigationController {
                    let chatListController = ChatListControllerImpl(context: self.context, location: .chatList(groupId: groupId), controlsHistoryPreload: false, enableDebugActions: false, animationInController: self.animationInController, animationOutController: self.animationOutController)
            ...
            case let .peer(peerData):
                ...
                case .chatList:
                    if case let .channel(channel) = peer.peer, channel.flags.contains(.isForum) {
                        ...
                        let contextController = ContextController(account: strongSelf.context.account, presentationData: strongSelf.presentationData, source: source, items: chatContextMenuItems(context: strongSelf.context, peerId: peer.peerId, promoInfo: promoInfo, source: .chatList(filter: strongSelf.chatListDisplayNode.mainContainerNode.currentItemNode.chatListFilter), chatListController: strongSelf, joined: joined) |> map { ContextController.Items(content: .list($0)) }, gesture: gesture, animationInController: strongSelf.animationInController, animationOutController: strongSelf.animationOutController)
                        strongSelf.presentInGlobalOverlay(contextController)
                    }
